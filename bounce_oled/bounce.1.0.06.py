...
def main():
    ...
    while True:
        ...
        if x <= 0: dx = iot_random.random_minmax(1, bounce_config.max_dxdy)

        if x >= (bounce_config.oled_width - 1): dx = - iot_random.random_minmax(1, bounce_config.max_dxdy)

        if y <= 0: dy = iot_random.random_minmax(1, bounce_config.max_dxdy)

        if y >= (bounce_config.oled_height - 1): dy = - iot_random.random_minmax(1, bounce_config.max_dxdy)
