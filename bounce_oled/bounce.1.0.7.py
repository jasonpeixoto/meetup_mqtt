...
def main():
    ...
    while True:
        ...
        if x <= 0: dx = bounce_random.random_max(bounce_config.max_dxdy)

        if x >= (bounce_config.oled_width - 1): dx = - bounce_random.random_max(bounce_config.max_dxdy)

        if y <= 0: dy = bounce_random.random_max(bounce_config.max_dxdy)

        if y >= (bounce_config.oled_height - 1): dy = - bounce_random.random_max(bounce_config.max_dxdy)
