...
def main():
    ...
    while True:
        ...
        if x <= 0: dx = iot_random.random_dx()

        if x >= (bounce_config.oled_width - 1): dx = - iot_random.random_dx()

        if y <= 0: dy = iot_random.random_dy()

        if y >= (bounce_config.oled_height - 1): dy = - iot_random.random_dy()
